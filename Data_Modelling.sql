...
SELECT o.Shipping_Mode, d.Name AS department_name, COUNT(o.Order_Id) AS order_count
FROM orders o
JOIN ordered_items oi ON o.Order_Id = oi.Order_Id
JOIN product_info pi ON oi.Item_Id = pi.Product_Id
JOIN department d ON pi.Department_Id = d.Id
...
WITH compliance AS (
	SELECT Order_Id, Shipping_Mode, Order_Status, Real_Shipping_Days, Scheduled_Shipping_Days,
		CASE
...
WITH cancelled_orders AS (
	SELECT Order_State, COUNT(*) AS cancelled
	FROM orders
...
SELECT t.Order_State,
       ROUND((c.cancelled / t.total) * 100, 2) AS cancellation_percentage
FROM total_orders t
LEFT JOIN cancelled_orders c ON t.Order_State = c.Order_State
